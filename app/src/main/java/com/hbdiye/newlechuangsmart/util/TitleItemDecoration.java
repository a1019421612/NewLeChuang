...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    if (null != mData.get(position).inital && !mData.get(position)
                            .inital.equals(mData.get(position - 1).inital)) {
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        mPaint.getTextBounds(mData.get(position).inital, 0, mData.get(position).inital.length(), mBounds);
        c.drawText(mData.get(position).inital, 
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {
            ...
            if (position == 0) {
                ...
                if (null != mData.get(position).inital && 
                        !mData.get(position).inital.equals(mData.get(position - 1).inital)) {
