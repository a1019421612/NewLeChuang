...
public class DashboardView extends View {
    ...
    private RectF mRectFProgressArca;
...
//        );
        mRectFProgressArc.set(
                mPadding + mSparkleWidth/2f*12,
                mPadding + mSparkleWidth/2f*12 ,
                getMeasuredWidth() - mPadding - mSparkleWidth / 2f-mSparkleWidth/2f*12,
                getMeasuredWidth() - mPadding - mSparkleWidth / 2f-mSparkleWidth/2f*12
        );
...
//            );
            ...
            canvas.drawCircle(point[0], point[1], 0, mPaint);
        } else {
            /**
             * 画进度圆弧(起始到信用值)
             */
...
//        }
        if (value>450){
            return 10 * degreePerSection ;
        }
        else if (value>430){
            return 8 * degreePerSection + 2 * degreePerSection / 20 * (value - 430);
        }else if (value>410){
            return 6 * degreePerSection + 2 * degreePerSection / 20 * (value - 410);
        }else if (value>390){
            return 4 * degreePerSection + 2 * degreePerSection / 20 * (value - 390);
        }else if (value>370){
            return 2 * degreePerSection + 2 * degreePerSection / 20 * (value - 370);
        }else {
            return 2 * degreePerSection / 20 * (value - 350);
        }
//        if (value > 700) {
//            return 8 * degreePerSection + 2 * degreePerSection / 250 * (value - 700);
//        } else if (value > 650) {
