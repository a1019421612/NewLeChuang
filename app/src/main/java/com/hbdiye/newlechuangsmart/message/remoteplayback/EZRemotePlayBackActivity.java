 ...
 * @warning 这里写本文件的相关警告
 */
package com.hbdiye.newlechuangsmart.message.remoteplayback;
...
import com.hbdiye.newlechuangsmart.remoteplayback.list.RemoteListUtil;
...
//                handleSearchEZDeviceFileSuccess();
    ...
    private void handleSearchEZCloudFileSuccess(Calendar seletedTime) {
        if (mEZMediaPlayer == null) {
            mEZMediaPlayer = EZOpenSDK.getInstance().createPlayer(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo());
            if (mEZMediaPlayer == null)
                return;
...
//        mRemotePlayBackCaptureRl.setVisibility(View.VISIBLE);
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    mCloudFileList = EZOpenSDK.getInstance().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(),starttime,endtime);
    ...
    private void searchEZCloudFileList(final Calendar seletedTime) {
        new Thread() {
            ...
            public void run() {
                try {
                    mEZCloudFileList = EZOpenSDK.getInstance().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(), mStartTime, mEndTime);
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    mEZDeviceFileList = EZOpenSDK.getInstance().searchRecordFileFromDevice(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(), startTime, endTime);
                    if (mEZDeviceFileList != null && mEZDeviceFileList.size() > 0) {
                    ...
                    mEZCloudFileList = EZOpenSDK.getInstance().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(), startTime, endTime);
