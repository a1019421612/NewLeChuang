package com.hbdiye.newlechuangsmart.common;
...
public abstract class HikAsyncTask<Params, Progress, Result> {
     ...
     * method can call {@link #publishProgress} to publish updates on the UI thread.
     *
     * @param params
     ...
     * <p>
     * Note: this function schedules the task on a queue for a single background thread or pool of
     ...
     * {@link android.os.Build.VERSION_CODES#DONUT}, this was changed to a pool of threads allowing
     * multiple tasks to operate in parallel. Starting
     * {@link android.os.Build.VERSION_CODES#HONEYCOMB}, tasks are back to being executed on a
     * single thread to avoid common application errors caused by parallel execution. If you truly
     * want parallel execution, you can use the {@link #executeOnExecutor} version of this method
     * with {@link #THREAD_POOL_EXECUTOR}; however, see commentary there for warnings on its use.
     * <p>
     ...
     * caller can keep a reference to it.
     * <p>
     * This method is typically used with {@link #THREAD_POOL_EXECUTOR} to allow multiple tasks to
     ...
     * <p>
     * <em>Warning:</em> Allowing multiple tasks to run in parallel from a thread pool is generally
     * <em>not</em> what one wants, because the order of their operation is not defined. For
     * example, if these tasks are used to modify any state in common (such as writing a file due to
     * a button click), there are no guarantees on the order of the modifications. Without careful
     * work it is possible in rare cases for the newer version of the data to be over-written by an
     * older one, leading to obscure data loss and stability issues. Such changes are best executed
     * in serial; to guarantee such work is serialized regardless of platform version you can use
     * this function with {@link #SERIAL_EXECUTOR}.
     * <p>
     * This method must be invoked on the UI thread.
     *
     * @param exec
     *            The executor to use. {@link #THREAD_POOL_EXECUTOR} is available as a convenient
     ...
     * execution of {@link #onProgressUpdate} on the UI thread. {@link #onProgressUpdate} will not
     * be called if the task has been canceled.
     *
     * @param values
