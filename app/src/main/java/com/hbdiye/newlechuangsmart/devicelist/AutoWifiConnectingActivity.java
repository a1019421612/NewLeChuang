...
//    private UnbindDeviceTriggerHelper mTriggerHelper;
    ...
    private int probeDeviceInfo(String deviceSerial) {
        mEZProbeDeviceInfo = EZOpenSDK.getInstance().probeDeviceInfo(serialNo,deviceType);
...
//        mTriggerHelper.onActivityResult(requestCode, resultCode, intent);
    ...
    private void initUI() {
        if (fromPage == FROM_PAGE_SERIES_NUM_SEARCH_ACTIVITY) {
            tv_title.setText(R.string.auto_wifi_title_add_device);
        } else {
            // 一切为了转圈
            // if (TextUtils.isEmpty(deviceType)) {
            tv_title.setText(R.string.auto_wifi_title_add_device2);
            // } else {
    ...
    private void start() {
       ...
       if(support_Wifi && support_sound_wave){
            mode = EZConstants.EZWiFiConfigMode.EZWiFiConfigSmart| EZConstants.EZWiFiConfigMode.EZWiFiConfigWave;
        }else  if (support_Wifi){
           mode = EZConstants.EZWiFiConfigMode.EZWiFiConfigSmart;
       }else if(support_sound_wave){
           mode = EZConstants.EZWiFiConfigMode.EZWiFiConfigWave;
       }
        EZOpenSDK.getInstance().stopConfigWiFi();
        EZOpenSDK.getInstance()
            .startConfigWifi(AutoWifiConnectingActivity.this, serialNo, wifiSSID, wifiPassword, mode,
...
//        WebUtils.openYsCloudIntro(this);
    ...
    private void lineConnectClick() {
        ...
        if (btnLineConnect.getVisibility() == View.VISIBLE) {
            tv_title.setText(R.string.auto_wifi_line_connect_title);
        } else if (TextUtils.isEmpty(deviceType)) {
            tv_title.setText(R.string.auto_wifi_network_add_device2);
        } else {
            tv_title.setText(R.string.auto_wifi_network_add_device1);
        }
